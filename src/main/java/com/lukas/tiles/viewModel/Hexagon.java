...
public class Hexagon {
    ...
    public Hexagon(int xCoordinate, int yCoordinate, double scale) {
        ...
        for (int i = 0; i < 6; i++) {
            x[i] = (x[i] + xCoordinate * 1.5) * scale * getRelativeScale();

            //Even row
            if (xCoordinate % 2 == 0) {
                y[i] = (y[i] + yCoordinate * 2 * dis) * scale * getRelativeScale();
            }
            //Uneven row
            else {
                y[i] = y[i] = (y[i] + yCoordinate * 2 * dis + dis) * scale * getRelativeScale();
            }
        }
    }
